...
const theForm = (targetForm, sort) => {
  ...
  switch (typeof targetForm) {
    case "number":
      form =
        targetForm >= 500 && targetForm <= 5000
          ? `${form}${targetForm}`
          : `${form}500`;
      break;
    case "string":
  ...
  form = `${form}" /> units</div><div><em>Ordered dose must be between 500 units and 5,000 units.</em></div><div>Sort by:</div><div><input type="radio" id="sortScore" name="sort" value="score" ${
    sort === "score" ? 'checked="true" ' : ""
  }/> <label for="sortScore">score (internal)</label></div><div><input type="radio" id="sortVCount" name="sort" value="vCount" ${
    sort === "vCount" ? 'checked="true" ' : ""
  }/><label for="sortVCount"> vial count</label></div><div><input type="radio" id="sortDiff" name="sort" value="diff" ${
    sort === "diff" ? 'checked="true" ' : ""
  }/> <label for="sortDiff">difference</label></div><div><input type="submit" value="Search" /></div></form>`;
  return form;
};
  ...
  if (req.method === "GET" && url.pathname === "/") {
    ...
    if (searchParams.has("target")) {
      ...
      try {
        ...
        res.end(
          `${preheader}KCentra Calculator${postheader}${theForm()}</body></html>`
        );
        return;
      }
      ...
      for (let i = 0, sl = sResults.length; i < sl; i++) {
        ...
        tResults = `${tResults}</td><td>${sum}</td><td>${sign}${Math.abs(
          diff
        )} (${sign}${(Math.abs(diff / reqNumber) * 100).toFixed(
          2
        )}%)</td></tr>`;
      }
      ...
      res.end(
        `${preheader}KCentra Calculator Results${postheader}${theForm(
          reqNumber,
          sortBy
        )}${tResults}</body></html>`
      );
    ...
    res.end(
      `${preheader}404${postheader}404: Resource Not Found</body></html>`
    );
    return;
  }
});
